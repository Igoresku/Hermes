...
Map_Abstraction::Map_Abstraction(int dimensions, int abstraction_size) : dimensions(dimensions), abstraction_size(abstraction_size) {
    hierarchy_size = 0;
    for (int i = dimensions; i > 1; i /= abstraction_size)
        hierarchy_size += 1;
} /// Map_Abstraction_Master : END

void Map_Abstraction::Create_Abstraction(Game_Map* game_map, Traversal_Type&& traversal_type) {
    ...
    map_abstractions[(int)traversal_type] = new Map_Abstraction_Level(0, 0, hierarchy_size, max_capacity);
